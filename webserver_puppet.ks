...
logvol  /usr        --vgname="$(hostname)_os" --size=10240 --name=usr --fstype ext4
logvol  /tmp        --vgname="$(hostname)_os" --size=10240 --name=tmp --fstype ext4
logvol  /var        --vgname="$(hostname)_os" --size=30720 --name=var --fstype ext4
logvol  /home       --vgname="$(hostname)_os" --size=10240 --name=home --fstype ext4
...
#useradd -u 1500 -d /home/user -m -s /bin/bash user
#passwd -l user
...
#mkdir -p /somedir/
#chown -R user:user /somedir
 
#yum -y install package1 package2 package3 

#maybe do some ulimit 
echo "APACHE_ULIMIT='ulimit -n 20480 -u 8192'" >> /etc/httpd/bin/envvars
echo "export APACHE_ULIMIT" >> /etc/httpd/bin/envvars
sed -i "s/^ULIMIT_MAX_FILES\=/\#ULIMIT_MAX_FILES\=/" /etc/httpd/bin/apachectl
...
/usr/bin/puppet agent -t --environment=install
/usr/bin/puppet agent -t --environment=build
/usr/sbin/puppetd -tv
